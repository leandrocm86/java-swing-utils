...
public class RelativeLayout implements LayoutManager2, java.io.Serializable
{
	...
	private final static float COMPONENT = -1.0f;

	//  See setRoundingPolicy() method
	// public final static int DO_NOTHING = 0;
	// public final static int FIRST = 1;
	// public final static int LAST = 2;
	private final static int LARGEST = 3;
	...
	/**
	 * Whether to fill the space available on the opposite axis.
	 */
	 ...
	 */
	public RelativeLayout(Axis axis)
	{
		this(axis, 0);
	}
	 ...
	 * axis using the specified gap between them, the gap on the borders and wether to fill the other axis.
	 * <p>
	 * All <code>RelativeLayout</code> constructors defer to this one.
	 * @param axis - HORIZONTAL (X-AXIS) or VERTICAL (Y_AXIS).
	 * @param gap - The gap between each component.
	 * @param borderGap - The gap on the borders.
	 * @param fill - Wether the components should fill the opposite axis.
	 */
	public RelativeLayout(Axis axis, int gap, int borderGap, boolean fill)
	{
		setAxis(axis);
		setGap(gap);
		...
		// For now it's not possible to customize rounding and alignment.
		// The code is ready though, but commented.
		setRoundingPolicy(LARGEST);
		setAlignment(CENTER);
	}
	 ...
	 *
	 * @param alignment - The constant float value defining the alignment.
	 */
	 ...
	 * @param fill - Wether the opposite axis must have its space filled with the components.
	 */
	 ...
	 * @param fillGap - The margin to be respected if {@link #fill} is set to TRUE.
	 */
	 ...
	 *
	 * Private for now, because currently rounding is not customizable.
	 */
	private void setRoundingPolicy(int roundingPolicy) {
	 ...
	 * 
	 * @param relativeSpace - Array with the divided spaces calculated.
	 * @param spaceRemaining - The space left from the division error.
	 */
